...
#pragma omp parallel for simd
    ...
    if(rank == world_size-1){
		MPI_Send(&d_t[start_segment], 1, MPI_FLOAT, lnb, 0, MPI_COMM_WORLD);
		MPI_Recv(&d_t[start_segment-1], 1, MPI_FLOAT, lnb, 0, MPI_COMM_WORLD, &stat);
	} else if (rank == 0){
		MPI_Send(&d_t[last_segment-1], 1, MPI_FLOAT, rnb, 0, MPI_COMM_WORLD);
		MPI_Recv(&d_t[last_segment], 1, MPI_FLOAT, rnb, 0, MPI_COMM_WORLD, &stat);
	} else {
		MPI_Send(&d_t[last_segment-1], 1, MPI_FLOAT, rnb, 0, MPI_COMM_WORLD);
		MPI_Recv(&d_t[start_segment-1], 1, MPI_FLOAT, lnb, 0, MPI_COMM_WORLD, &stat);
		MPI_Send(&d_t[start_segment], 1, MPI_FLOAT, lnb, 0, MPI_COMM_WORLD);
